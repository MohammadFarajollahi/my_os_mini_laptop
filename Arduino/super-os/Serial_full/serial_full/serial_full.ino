...
void setup() {
  ...
  lv_obj_center(label_save);
    ...
    lv_obj_add_event_cb(fileNameInput, [](lv_event_t * e) {
...
// تابع پردازش فریم (بسته به کتابخانه نمایشگر شما)
void my_flush_cb(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_map) {
    // کد پردازش فریم را در اینجا قرار دهید
    // این تابع باید برای نمایش هر فریم جدید در نمایشگر شما سفارشی شود
}