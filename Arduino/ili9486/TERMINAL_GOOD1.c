...
void setup() {
 ...
 // Create a textarea (terminal) to display text
  ...
  lv_obj_align(btn_clear, LV_ALIGN_BOTTOM_LEFT, 20, -10);  // موقعیت دکمه پاک کردن
  ...
  lv_obj_align(btn_color, LV_ALIGN_BOTTOM_MID, 0, -10);  // موقعیت دکمه تغییر رنگ
  lv_obj_set_size(btn_color, 100, 40);

  // Label for the color change button
  label_color = lv_label_create(btn_color);
  lv_label_set_text(label_color, "Change Color");
  ...
  lv_obj_align(btn_baud, LV_ALIGN_BOTTOM_RIGHT, -20, -10);  // موقعیت دکمه تغییر baud rate
